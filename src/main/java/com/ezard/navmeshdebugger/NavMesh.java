		...
		if (maxIndex == 0) {
			...
			for (int i = maxIndex - 2; i >= 0; i--) {
				for (NavPoint.Link l : path[i + 1].links) {
					if (l.target.index == i + 1 && !(l.target.position.y > path[i + 1].position.y && l.linkType == LinkType.FALL) &&
							!(l.target.position.y < path[i + 1].position.y && l.linkType == LinkType.JUMP)) {
						path[i] = l.target;
						break;
					}
				}
